...
\begin{algorithmic}
\Require $\mathbf{y}^{\prime\prime}$ \Comment{Second derivative array}
\State $n \gets \text{len}(\mathbf{y}^{\prime\prime})$
\State $\texttt{regions} \gets \texttt{[]}$
\State $i \gets 1$
\While {$i < n$}
  \If{$\mathbf{y}^{\prime\prime}_i < \omega \cdot \sigma_{\text{bg}}$}
    \State $\texttt{start} \gets i$
    \State $i \gets i+1$
    \While {$\mathbf{y}^{\prime\prime}_i < \omega \cdot \sigma_{\text{bg}}$}:
      \State $i \gets i+1$
    \EndWhile
    \State $\texttt{stop} = i$
