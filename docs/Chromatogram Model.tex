...
\EndFor
\Function{SmoothnessPenalty}{$\overline{\mathbf{y}}^s, i, s, n$}
...
\State $\mathbf{z} \gets \text{mean}(\mathbf{y}) \cdot \mathbf{1}$\Comment{initial baseline guess}
\State $\mathbf{r} \gets \mathbf{y} - \mathbf{z}$\Comment{initial residuals}
\State $\mathbf{w} \gets \mathbf{1}$ \Comment{initialize all weights to 1}
\State $\text{prev\_loss} \gets \infty$
\While{not converged}
\State $\mathbf{W} \gets \text{diag}(\mathbf{w})$ \Comment{create diagonal matrix of weights}
\State $\mathbf{z} \gets (\mathbf{W} + s \cdot \mathbf{D}^T\mathbf{D})^{-1}\mathbf{W}\mathbf{y}$ \Comment{solve normal equation}
\State $\mathbf{r} \gets \mathbf{y} - \mathbf{z}$ \Comment{calculate updated residuals}
\State $\text{loss} \gets C(\mathbf{r},\mathbf{w},\mathbf{z})$ \Comment{calculate loss}
...
\State update $\mathbf{w}$ \Comment{update according to Eqn \ref{eqn:weights}}
\EndWhile
\end{algorithmic}
\end{algorithm}
...
\begin{algorithmic}
\Require $\mathbf{d}$ \Comment{Second derivative array}
\State $n \gets \text{len}(\mathbf{d})$
\State $\texttt{regions} \gets \texttt{[]}$
\State $i \gets 1$
\While {$i < n$}
  \If{$\mathbf{d}_i < \omega \cdot \sigma_{\text{bg}}$}
    \State $\texttt{start} \gets i$
    \State $i \gets i+1$
    \While {$\mathbf{d}_i < \omega \cdot \sigma_{\text{bg}}$}:
      \State $i \gets i+1$
    \EndWhile
    \State $\texttt{stop} = i$
...
\subsubsection{Region Contraction}
\begin{algorithm}[t]
\caption{Region contraction}\label{alg:regcont}
\begin{algorithmic}
\State $y \gets x$
\end{algorithmic}
\end{algorithm}
