...
for i, system_name in enumerate(system_names):
    ...
    ax.legend(title="Compound", loc="lower left", bbox_to_anchor=(0, 0))
...
plt.tight_layout()
plt.show()
plt.savefig(f"{metric}_over_Time_by_System.png")
...
df_cori = df[
    (df["name"] == "TS-576B")
    & (df["injection_name"] == "tetracinib_degradation_standard")
]
